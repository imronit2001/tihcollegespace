...
if($subject=="all"){
    if($dateprint=="all"){
        $query="SELECT * from schedule_class WHERE stream='$stream' and sem='$sem'  and (section='$section' or section='Combined')  ORDER BY date desc";
      }
      else
      if($dateprint=="today"){
        ...
        $query = "SELECT * FROM schedule_class WHERE stream='$stream' and sem='$sem' and (section='$section' or section='Combined') and date='$dateupto' ORDER BY date desc";
      }
      else
    ...
    if($dateprint=="all"){
        $query="SELECT * from schedule_class WHERE stream='$stream' and sem='$sem' and subject='$subject'  and (section='$section' or section='Combined') ORDER BY date desc";
      }
      else
      if($dateprint=="today"){
        $date=date_create(date("Y-m-d"));
        date_add($date,date_interval_create_from_date_string("0 days"));
        $dateupto=date_format($date,"Y-m-d");
        $query = "SELECT * FROM schedule_class WHERE stream='$stream' and sem='$sem' and subject='$subject'   and (section='$section' or section='Combined') and date='$dateupto' ORDER BY date desc";
      }
      else
      if($dateprint=="tommorow"){
        $date=date_create(date("Y-m-d"));
...
// $query="select * from schedule_class where stream='$stream'";
// echo $query;
       ...
       <table class="table table-hover">
            ...
            <div id="sem" style="display:none"><?php //echo $semesters_id; ?>
                ...
                <select name="" id="FilterDateC" onchange="FilterDateC(this.value)">
                    ...
                    <option value="tommorow" <?php if($dates=="tommorow"){echo 'selected';} ?>>Tommorow</option>
                    <option value="week" <?php if($dates=="week"){echo 'selected';} ?>>This Week</option>
                    <option value="month" <?php if($dates=="month"){echo 'selected';} ?>>This Month</option>
                  </select>
                </th>
                <th>Time</th>
            </tr>
